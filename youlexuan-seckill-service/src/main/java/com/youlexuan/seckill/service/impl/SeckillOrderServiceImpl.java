...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long id, String userId) {
		...
		//1\
		TbSeckillGoods seckillGoods = (TbSeckillGoods) redisTemplate.boundHashOps("seckill_goods_list_key").get(id);
		...
		redisTemplate.boundHashOps("seckill_goods_list_key").put(id,seckillGoods);
		if(seckillGoods.getStockCount()==0){
			seckillGoodsMapper.updateByPrimaryKey(seckillGoods);
			redisTemplate.boundHashOps("seckill_goods_list_key").delete(id);
		}
	...
	public TbSeckillOrder searchSeckillOrderByUserId(String userId) {
		return (TbSeckillOrder) redisTemplate.boundHashOps("seckill_order_key").get(userId);
	}
	...
	public void saveOrderFromRedisToDb(String userId, String trade_no) {
		...
		//1、从Redis中拿到订单信息
		TbSeckillOrder seckillOrder = (TbSeckillOrder) redisTemplate.boundHashOps("seckill_order_key").get(userId);
		//补充下数据安全性的判断
		if(seckillOrder==null){
	...
	public void resetOrderFromRedis(String userId) {
		...
		//2、
		TbSeckillGoods seckillGoods = (TbSeckillGoods) redisTemplate.boundHashOps("seckill_goods_list_key").get(seckillOrder.getSeckillId());
		if(seckillGoods!=null){
			seckillGoods.setStockCount(seckillGoods.getStockCount()+1);
			redisTemplate.boundHashOps("seckill_goods_list_key").put(seckillOrder.getId(),seckillGoods);
		}
	...
	public void deleteOrderFromRedis(String userId, Long orderId) {
		...
		if(seckillOrder!=null &&
			...
			redisTemplate.boundHashOps("seckill_order_key").delete(userId);//删除缓存中的订单
			//恢复库存
			//1.从缓存中提取秒杀商品
			TbSeckillGoods seckillGoods=(TbSeckillGoods)redisTemplate.boundHashOps("seckill_goods_list_key").get(seckillOrder.getSeckillId());
			if(seckillGoods!=null){
				seckillGoods.setStockCount(seckillGoods.getStockCount()+1);
				redisTemplate.boundHashOps("seckill_goods_list_key").put(seckillOrder.getSeckillId(), seckillGoods);//存入缓存
			}
		}
	}
